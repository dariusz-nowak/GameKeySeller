...
def checkExistingGamesInList(newGamesList, existingGamesList):
    ...
    for newGame in newGamesList:
        ...
        for existingGame in existingGamesList:
            if newGame['title'] == existingGame['title'] and newGame['current price'] != existingGame['current price']: 
...
def saveGamesListToFile(games):
    with open("docs/games.txt", "w", encoding='utf8') as file: 
        file.write(str([game for game in games if game['status'] != "deleted"]))
    return games
...
def checkGames():
    ...
    # return sortGamesInList(saveGamesListToFile(checkExistingGamesInList(loadGamesFromURL(url), loadGamesListFromFile())))

    with open("docs/GGdealsGamesList.txt", "r", encoding='utf8') as file: return literal_eval(file.read())