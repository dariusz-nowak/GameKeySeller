...
# cursor.execute(f"UPDATE games SET old_price = 176.43, current_price = 10.03, time_modyfied = strftime('%Y-%m-%d %H-%M-%S','now') WHERE id = 70")
# cursor.execute(f"UPDATE games SET old_price = 205.74, current_price = 30.01, time_modyfied = strftime('%Y-%m-%d %H-%M-%S','now') WHERE id = 71")
# cursor.execute(f"UPDATE games SET title = 'WATAFAK' WHERE id = 74")
# cursor.execute(f"UPDATE games SET title = 'WATAFAK' WHERE id = 75")
# connection.commit()
