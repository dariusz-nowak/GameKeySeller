...
from backend.database.databaseActions import loadPlatforms, loadFilteredAdverts

def loadAdverts(self, loadTableExporter):
    filters = {
    ...
    html = """
            <div class="adverts-raport big window">
                <h1>Oferty</h1>
                <h3 onClick="loadAdvertsFilterForm()">Filtry v</h3>
                <form action="/filtration-load-adverts-raport" onsubmit="return checkForm()" method="get" class="hide">
                    <div class="filters">
                        <div>
    ...
    salesPlatforms = loadPlatforms('sales_platforms')
    html += '<div>'
    html += '<label for="sale-platform">Platforma sprzedaży:</label>'
    html += '<select id="sale-platform" name="sale-platform">'
    html += '<option value=''></option>'
    for platform in salesPlatforms: html += f"<option value='{platform['name']}' {'selected' if platform['name'] == filters['sale platform'] else ''}>{platform['name']}</option>"
    html += '</select></div>'
    ...
    for sale in loadFilteredAdverts(filters):
